...
def analyze_employees(employees: List[Employee], analyzer: EmployeeAnalyzer) -> List[Dict]:
...
def run_workflow(project_requirement: ProjectRequirement) -> Dict:
    ...
    try:
        ...
        if not employee_bookings:
            logger.warning("No employee bookings found, assuming all employees are available")
        else:
        ...
        available_employees = [emp for emp in all_employees if emp.empCode not in booked_employees]
        
        logger.info(f"Filtered from {len(all_employees)} to {len(available_employees)} available employees based on bookings")
